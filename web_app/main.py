...
class WebInterface:
    ...
    def _build_report_context(self, task):
        ...
        # 4. 兼容LangChain pipeline结构（detection/analysis/repair/validation）
        for agent_key in ['detection', 'analysis', 'repair', 'validation']:
            agent_result = results.get(agent_key, {})
            if isinstance(agent_result, dict):
                for key in ['issues', 'defects', 'problems', 'all_issues']:
                    if key in agent_result and isinstance(agent_result[key], list):
                        for issue in agent_result[key]:
                            issues.append(issue)

        # 5. 代码前后对比，优先用repair agent
        code_before = ''
        code_after = ''
        repair_result = results.get('repair', {})
        if isinstance(repair_result, dict):
            code_before = repair_result.get('code_before', '')
            code_after = repair_result.get('code_after', '')
            # 兼容修复建议
            for k in ['fix_suggestion', 'fixed_code', 'code_after']:
                if not code_after and k in repair_result and isinstance(repair_result[k], str) and len(repair_result[k]) > 10:
                    code_after = repair_result[k]
        # fallback: detection agent原始代码
        if not code_before:
            detection_result = results.get('detection', {})
            if isinstance(detection_result, dict):
                code_before = detection_result.get('code_before', '')
        # fallback: analysis agent结构
        if not code_before:
            analysis_result = results.get('analysis', {})
            if isinstance(analysis_result, dict):
                code_before = analysis_result.get('code', '')

        # 自动标记修复状态：如果有修复结果，所有问题都视为已修复
        repair_result = results.get('repair', {})
        repair_success = False
        if isinstance(repair_result, dict):
            repair_success = bool(repair_result.get('repair_results'))
        for issue in issues:
            if repair_success:
                issue['fixed'] = True
            elif 'fixed' not in issue:
                issue['fixed'] = False

        total_issues = len(issues)
        fixed_issues = sum(1 for i in issues if i.get('fixed', False))
    ...
    def _setup_routes(self):
        ...
        @self.app.post("/api/langchain/upload")
        async def langchain_upload(files: list[UploadFile] = File(...)):
            """上传文件/文件夹，保存到临时目录，返回临时路径"""
        ...
        # 新增：LangChain流水线分析接口
        @self.app.post("/api/langchain/pipeline")
        async def langchain_pipeline(request: dict = Body(...)):
            """统一分析-修复-测试入口，支持临时目录，无需本地路径校验，自动创建任务并推送进度"""
            from src.agents.langchain_pipeline_agent import pipeline_run
            import asyncio
            repo_path = request.get("repo_path", "")
            preferred_model = request.get("preferred_model", "deepseek")
            if not repo_path or not os.path.exists(repo_path):
                return {"error": "请上传文件/文件夹或输入有效路径"}
            ...
            try:
                ...
                results = await pipeline_run(repo_path)
